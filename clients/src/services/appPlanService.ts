...
import { multiStagePromptSystem, type ResearchPromptInput } from './multiStagePromptSystem';
import { type AppPlanInput } from '@siso-public/shared-types';
...
function createIndustrySpecificFeatures(data: AppPlanData) {
  ...
  return {
    features: selectedFeatures.map((feature, index) => ({
      id: `industry_${index}`,
      title: feature.title,
    ...
    researchResults: {
      industryAnalysis: [
        `${data.industry} market shows strong growth potential for digital solutions`,
        `Industry-specific features tailored to ${data.company}'s needs`,
        `Competitive analysis indicates opportunities for differentiation`
      ],
      companyAnalysis: [
        `${data.company} well-positioned for digital transformation`,
        `Custom solution will address specific business requirements`,
        `Strategic implementation approach recommended`
      ],
      techRecommendations: [
        `Modern technology stack for ${data.industry} applications`,
        `Scalable architecture supporting business growth`,
        `Security and compliance considerations addressed`
      ],
      marketOpportunities: [
        `${data.industry} digitization creating new opportunities`,
